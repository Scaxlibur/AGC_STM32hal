...
uint32_t ADS1256ReadData(uint8_t channel)  
{
	...
	uint8_t data3=0;
	uint32_t sum=0;
	ADS1256_CMD_t cmd1 = ADS1256_CMD_SYNC;
	ADS1256_CMD_t cmd2 = ADS1256_CMD_WAKEUP;
	ADS1256_CMD_t cmd3 = ADS1256_CMD_RDATA;
	//uint8_t rdata[3];
	while(ADS1256_DRDY);//
	ADS1256WREG(ADS1256_MUX,channel);
	CS_0();
	//读数据前依次写入SYNC、WAKEUP、RDATA
	HAL_SPI_Transmit(&hspi2,(uint8_t *)&cmd1,1,100);
	HAL_SPI_Transmit(&hspi2,(uint8_t *)&cmd2,1,100);	               
	HAL_SPI_Transmit(&hspi2,(uint8_t *)&cmd3,1,100);
	//接收数据依次存入data中
